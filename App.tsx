...
import { TimelineEvent, MedicalRecordData, PatientProfile } from './types';
import { 
...
const App: React.FC = () => {
  ...
  const handleGenerate = async () => {
    ...
    const result = await generateAdmissionNote(payload);
  ...
  const handleFullRefine = async (customInstruction?: string) => {
    ...
    const payload = getPayload();
    const result = await refineFullNote(payload, generatedNote, instruction);
    updateHistory(result);
  ...
  const handleRefine = async () => {
    ...
    setIsRefining(true);
    const payload = getPayload();
    const newSegment = await refineNoteSegment(payload, generatedNote, selectedText, refinementInstruction);
    ...
    <div className="h-screen w-screen flex flex-col bg-slate-50 text-slate-800 font-sans overflow-hidden">
        ...
        <header className="h-14 bg-blue-900 text-white flex items-center px-4 justify-between shadow-md z-20 flex-shrink-0">
            <div className="flex items-center space-x-2">
                ...
                <h1 className="font-bold text-lg">Smart Medical Record</h1>
            </div>
                ...
                <div className="w-[70%] h-full p-4 relative overflow-hidden flex flex-col">
                  <div className="flex-1 relative">
                    <textarea 
                      ...
                      placeholder="The generated medical admission note will appear here..." 
                    />
